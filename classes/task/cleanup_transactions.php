...
class cleanup_transactions extends \core\task\scheduled_task {
    ...
    public function execute() {
        ...
        $pending_retention_days = (int)get_config('enrol_sepay', 'pending_retention_days');
        if ($pending_retention_days < 1) {
            $pending_retention_days = 30;
        }
        $pending_cutoff = time() - ($pending_retention_days * 86400);
        $DB->execute(
            "UPDATE {enrol_sepay_transactions}
                SET status = 'rejected', timeprocessed = :now
              WHERE status = 'pending'
                AND timecreated < :cutoff",
            ['now' => time(), 'cutoff' => $pending_cutoff]
        );
        mtrace('Đã từ chối các giao dịch pending quá ' . $pending_retention_days . ' ngày.');
        ...
        foreach ($old_transactions as $transaction) {
            if ($strategy === 'archive') {
                ...
                try {
                    ...
                    unset($archive_record->id); // Để DB tự động cấp ID mới.
